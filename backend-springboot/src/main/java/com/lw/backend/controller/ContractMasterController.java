...
public class ContractMasterController {
    ...
    private final ContractMasterMapper mapper;

    @PostMapping
    public Map<String, Object> create(@RequestBody ContractMaster data) {
        mapper.insert(data);
        return successWithData(data);
    }

    @PutMapping("/{id}")
    public Map<String, Object> update(@PathVariable String id, @RequestBody ContractMaster data) {
    ...
    public Map<String, Object> page(@RequestParam(defaultValue = "1") long pageNo,
        ...
        LambdaQueryWrapper<ContractMaster> wrapper = new LambdaQueryWrapper<ContractMaster>()
                .like(StringUtils.hasText(contractId), ContractMaster::getContractId, contractId)
                .eq(StringUtils.hasText(customerId), ContractMaster::getCustomerId, customerId)
                .orderByDesc(ContractMaster::getUpdatedAt);
        IPage<ContractMaster> result = mapper.selectPage(new Page<>(pageNo, pageSize), wrapper);
