...
- 每次新增数据表后，必须立即在本地数据库同步建表（执行对应DDL并验证可查询），再继续联调或提交。
...
- 2026-03-29: 生产协同模块新增并打通 6 个页面（排产工作区/织造订单/定型订单/裁网订单/插接订单/二次定型订单）；后端新增 `GET /api/schedule/workbench`、重构 `POST /api/schedule/merge` 到 `prd_weaving_process + map_order_weaving`，并新增 5 个工序订单分页接口（`/api/production/*-orders`）；订单明细补齐 `air_permeability` 链路并支持“同合同主单追加明细”录入；`LW.sql` 同步新增 `order_detail.air_permeability` 与 `prd_weaving_process.actual_width`。- 2026-03-29: 调整排产与生产审核状态流转：排产成功后主单/明细状态更新为 `2(生产审核中)` 并进入生产审核页；`POST /api/order-masters/{id}/production-review` 改为仅受理状态 `2` 订单，审核通过后主单/明细状态更新为 `3(织造中)`；前端 `ProductionReview.vue` 按状态 `2` 查询并执行“审核通过并流转织造”，`ScheduleWorkbench.vue` 成功提示改为“进入生产审核区”，订单状态字典同步更新为 `2生产审核中/3织造中`（含订单追踪明细状态展示）。- 2026-03-29: 细化“排产→审核→织造”流转规则：`/api/order-masters/{id}/production-review` 改为按“存在待审核明细(detail_status=2)”校验并流转到织造(detail_status=3)；排产与审核后均按明细聚合回写主单状态（无织造明细=1审核中，任一明细进入织造及后续=3生产中，全部明细=5时主单=6已完结）；`OrderQueryController` 的主单状态返回同步按该规则聚合；前端 `ProductionReview.vue` 保留审核按钮并改为展示“存在待审核明细”的订单。- 2026-03-29: 修正订单状态字典展示为标准 0~6 七种：`0草稿、1审核中、2待排产、3生产中、4部分入库、5已发货、6已完结`；移除前端状态文案中的“织造中/生产审核中”替代词，避免订单追踪页出现非标准终状态。- 2026-03-29: 区分主单状态与明细工序状态展示：订单总状态继续保持标准 0~6 字典，订单追踪明细状态字典将 `3` 调整为“正在织造”，避免与总状态“生产中”语义冲突。- 2026-03-29: 新增 `order_detail.weaving_mode_status` 字段并打通前后端：数据库字段值严格定义 `0待合批/1合批织造/2单网织造`；录入明细默认写入 `0`；排产工作台下发时按勾选明细数自动写入 `1`（多条合并）或 `2`（单条织造）。同时前端状态字典严格对齐数据库：主单状态固定 0~6，`detail_status` 固定 1~5。- 2026-03-29: 将 `order_detail.detail_status` 流转规则调整为三段：`0待排产 -> 1待审核 -> 2织造中`。联动改动包括：明细录入默认值改为 `0`；排产工作区仅拉取 `detail_status=0` 明细并在排产后写入 `1`；生产审核通过将 `1` 批量更新为 `2`；订单追踪前端状态字典严格对齐该三段定义；`LW.sql` 与本地库 `order_detail` 默认值/检查约束同步为 `(0,1,2)`。- 2026-03-29: 按新业务口径重定义 `order_detail.detail_status` 为三段流转（`0待排产/1待审核/2织造中`），并同步后端流转链路：明细创建默认 `0`，排产后改 `1`，生产审核通过改 `2`；排产工作区筛选条件改为 `detail_status=0`，生产审核页改为识别 `detail_status=1`。同时主单状态聚合改为：存在任一 `detail_status>=2` 则主单 `3生产中`，否则 `1审核中`。- 2026-03-29: 订单录入规则调整为“仅录入明细订单”，后端 `OrderQueryController` 改为按 `contractNo + D` 自动生成订单主表号（不再使用手工/时间戳订单号），明细号生成规则改为 `orderNo-序号`；前端 `OrderManage.vue` 移除订单号输入；`OrderList.vue` 主表新增合同号列用于追踪展示。- 2026-03-29: 新增织造报工闭环：`LW.sql` 增加 `prd_weaving_report`（机台/操作工/原料批次/张力参数/实际产出/开完工时间）；后端 `ProductionOrderController` 新增 `GET/POST /api/production/weaving-orders/{batchNo}/report`，提交报工时同步更新 `prd_weaving_process` 状态（开工=2、完工=3）并在完工后自动激活 `prd_setting_process`（待定型）。前端 `WeavingOrders.vue` 新增“报工”按钮与弹窗表单。- 2026-03-29: 修复织造报工“无法提交”问题：`ProductionOrderController` 将参数校验异常统一改为 `BizException`（避免 500 空白错误）；报工时间解析兼容 `yyyy-MM-ddTHH:mm:ss` 与 `yyyy-MM-dd HH:mm:ss`；并在报工接口增加 `prd_weaving_report` 自动建表兜底（`CREATE TABLE IF NOT EXISTS`），避免本地库未执行 DDL 导致报工失败。- 2026-03-29: 调整织造报工为“严格依赖本地数据库结构”：移除 `ProductionOrderController` 中自动建表逻辑，不再在接口执行 DDL；改为启动报工接口时检查 `prd_weaving_report` 是否存在，若缺失则返回业务错误“请先执行 LW.sql 同步数据库结构”。- 2026-03-29: 新增定型部门报工表 `prd_setting_report`：字段包含 `setting_batch_no`、`weaving_batch_no`(UNIQUE)、`operator_id`、`actual_temp`、`setting_duration`、`shrink_rate` 及审计时间；同步更新 `LW.sql` 清表顺序与建表定义，并已在本地数据库 `lw_db` 完成建表与结构回查。- 2026-03-29: 新增裁网部门报工表 `prd_cutting_report`：字段包含 `cut_batch_no`、`setting_batch_no`、`detail_id`、`operator_id`、`actual_cut_len`、`actual_cut_wid`、`waste_area` 及审计时间；同步更新 `LW.sql` 清表顺序，并已在本地数据库 `lw_db` 完成建表与 `SHOW CREATE TABLE` 回查。- 2026-03-29: 新增插接部门报工表 `prd_splicing_report`：字段包含 `splice_batch_no`、`cut_batch_no`(UNIQUE)、`operator_id`、`splice_type`、`joint_strength` 及审计时间；同步更新 `LW.sql` 清表顺序，并已在本地数据库 `lw_db` 完成建表与 `SHOW CREATE TABLE` 回查。- 2026-03-29: 新增二次定型（成品检测）报工表 `prd_sec_setting_report`：字段包含 `final_batch_no`、`splice_batch_no`(UNIQUE)、`operator_id`、`final_length`、`final_width`、`mesh_defect_info`、`qc_trigger_flag` 及审计时间；同步更新 `LW.sql` 清表顺序，并已在本地数据库 `lw_db` 完成建表与 `SHOW CREATE TABLE` 回查。- 2026-03-29: 登录改造为数据库真实鉴权：后端新增 `AuthController`（`POST /api/auth/login`、`GET /api/auth/me`、`POST /api/auth/logout`），基于 `sys_user + sys_user_role` 校验账号、状态与角色，密码支持明文与 BCrypt；前端 `auth.js` 移除写死账号映射，改为调用后端鉴权接口并持久化后端返回的 token/role/username；`Login.vue` 改为真实账号登录文案与异常提示。- 2026-03-29: 新增系统管理模块（用户管理）并启用真实角色维护：后端新增 `SystemUserController` 提供 `GET /api/system/roles`、`GET/POST/PUT/DELETE /api/system/users`，支持用户录入、分页查询、状态调整与角色分配；前端新增页面 `SystemUserManage.vue`，并接入路由 `/system/users`、左侧菜单“系统管理”、RBAC 映射（仅 `SYSTEM_ADMIN` 可见）。- 2026-03-29: 按“用户表整合鉴权+员工信息+小程序登录”扩展 `sys_user`：`status` 列调整为 `is_active`，新增 `phone`、`dept_id`、`open_id`、`union_id`，并补充唯一索引/状态约束；后端 `SysUser` 实体映射改为 `@TableField("is_active")` 保持兼容，系统用户管理接口与页面同步支持上述字段录入与查询；本地数据库已执行 `ALTER TABLE sys_user` 同步结构。- 2026-03-30: 五张工序报工表 `prd_weaving_report`、`prd_setting_report`、`prd_cutting_report`、`prd_splicing_report`、`prd_sec_setting_report` 统一新增 `operator_id -> sys_user.user_id` 外键约束（ON UPDATE CASCADE, ON DELETE RESTRICT）；`LW.sql` 与本地数据库 `lw_db` 已同步并通过 `information_schema.KEY_COLUMN_USAGE` 校验。