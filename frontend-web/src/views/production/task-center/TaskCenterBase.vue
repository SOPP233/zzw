  ...
  <div class="page">
    <el-card shadow="never">
      ...
      <el-form :inline="true" class="mb-12" @submit.prevent>
        ...
        <el-form-item v-if="!fixedProcessType" label="工序类型">
...
const fixedProcessType = computed(() => Number.isInteger(props.processType) && props.processType > 0);
const pageTitle = computed(() => props.title || (fixedProcessType.value ? `${PROCESS_TYPE_MAP[props.processType]}任务中心` : "工序任务中心"));
...
const query = reactive({
  batchId: "",
  processType: fixedProcessType.value ? props.processType : undefined
});

watch(
  () => props.processType,
  (newValue) => {
    query.processType = Number.isInteger(newValue) && newValue > 0 ? newValue : undefined;
    fetchTasks();
  }
);
